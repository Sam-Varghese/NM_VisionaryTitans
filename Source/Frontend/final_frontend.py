...
img = Image.open(
    r'C:\Users\Lenovo\Desktop\WEB_DEVELOPMENT\KAVACH\Source\Frontend\logo_title.jpg')
layout = "wide"
st.set_page_config(page_title="Cyber Security App",
                   page_icon=img, layout=layout)
...
def cctv_footages():
    ...
    st.markdown("<h3 style='text-align: center;'>Video Player</h3>",
...
def report():
    df = pd.read_csv(r"C:\Users\Lenovo\Desktop\WEB_DEVELOPMENT\KAVACH\Source\Frontend\dataset.csv")
...
def main_app():
    st.markdown("<h2 style='text-align: center ;'>NM_VisionaryTitans</h2>",
                unsafe_allow_html=True)

    # Check authentication status
    if st.session_state.get("isAuthenticated"):
        selected = option_menu(menu_title=None,
                               options=['CCTV Footages', 'Incidents Database',
                                        'Programmatic Actions', 'Manual Actions', 'Report'],
                               icons=['camera-fill', 'database-check',
                                      'chevron-double-right', 'clock-fill', 'file-earmark-text'],
                               default_index=0,
                               orientation='horizontal')
...
def login_app():
    ...
    if choice == "Home":
        st.markdown("<h2 style='text-align: center;'>HOME</h2>",
                    unsafe_allow_html=True)
