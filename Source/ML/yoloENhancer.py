...
class AnomalousMeans:
    ...
    def checkMeanAnomaly(self):
        ...
        if(p_value < self.threshold_value):
            ...
            self.alertsInstance.initiateMediumLevelAlert("Means differ significantly in {} as p value is {} abd threshold is {}.".format(self.detectionField, p_value, self.threshold_value))
...
class OutlierDetector:
    ...
    def addTrainingData(self, data_point):
        ...
        if(data_point > 10 and self.detectionField == "Speed"):
            self.alertsInstance.initialMediumLevelAlert("Data point for speed too high")
    ...
    def detectOutliers(self):
        ...
        self.threshold = np.mean(lof_scores)*self.anomalyThreshold
        # outliers = self.data[np.logical_or(lof_scores > self.threshold, lof_scores < self.threshold*(-1))]
        # outliers = self.data[lof_scores > self.anomalyThreshold]
        outliers = lof_scores[lof_scores > self.threshold]
        if(len(outliers) != 0):
            self.alertsInstance.initiateMediumLevelAlert("Detected Outliers in {} as threshold is {}.".format(self.detectionField, self.threshold/self.anomalyThreshold))
            console.print("Neighbours count: ", self.neighbours_count)
            console.print("Outlying values: ", outliers/self.anomalyThreshold)
            ...
            console.print("Max values for {} and the threshold is ".format(self.detectionField), np.max(lof_scores), self.threshold)
            return False
...
class DBSCAN:
    ...
    def autoAdjustParameters(self, sample_radius: float):
        ...
        self.min_pts = 3
        self.core_pt_threshold = len(self.recentObjectsDetectedCoordinates)/1.5
...
class Alerts:
    def __init__(self, field: str):
        ...
        self.previousAlertTime = time.time()
        self.alertInterval = 5 # To prevent alerts from getting spoken continuosly
        self.field = field

    def text_to_speech(self, text: str):
        if(time.time() - self.previousAlertTime >= self.alertInterval):
...
if __name__ == "__main__":
    ...
    vehicleCountAlert = Alerts("Vehicle Count")
    anomalousCountMeans = AnomalousMeans(vehicleCountAlert) # For checking anomalous changes in the count of vehicles. This function compares the mean values of all data points captured, so it should run relatively less number of times
    ...
    stayTimeAlertSystem = Alerts("Stay Time Alerts")
