# Yolov7 video processing in real time, with live video format
...
class Alerts:
    ...
    def autoInitiateAlert(self, cause):
        ...
        for cas in highAlertCause:
            if cas in cause:
                self.text_to_speech("Generating high level alert because "+ cause)
                self.initiateHighLevelAlert(cause)
        for cas in mediumAlertCause:
            if cas in cause:
                self.text_to_speech("Generating medium level alert because "+ cause)
                self.initiateMediumLevelAlert(cause)
        for cas in lowAlertCause:
            if cas in cause:
                self.text_to_speech("Generating low level alert because "+ cause)
